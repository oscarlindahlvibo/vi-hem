-- Repair migration for deployments where the invoice-tools migration was
-- recorded as applied without creating its RPC functions.

ALTER TABLE public.vihem_installment_schedule
  ADD COLUMN IF NOT EXISTS invoice_id uuid
    REFERENCES public.vihem_invoices(id) ON DELETE RESTRICT;
...
BEGIN
  ...
  INSERT INTO public.vihem_installment_audit_log (
    ...
    v_plan.organisation_id, v_plan.id, 'invoice_draft_created',
    jsonb_build_object('invoice_id', v_invoice.id, 'schedule_id', v_schedule.id),
    auth.uid()
  );
