-- Tillåt negativa kvittorader för rabatt, avdrag och provision.
-- Slutbeloppet måste fortfarande vara positivt innan ett ekonomiskt kvitto skapas.
...
BEGIN
  ...
  LOOP
    ...
    line_gross := round(COALESCE((line_row->>'amount')::numeric, 0), 2);
  ...
  END LOOP;
  IF total <= 0 THEN RAISE EXCEPTION 'Kvitto saknar ett positivt belopp.'; END IF;
