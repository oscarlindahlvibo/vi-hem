DROP POLICY IF EXISTS "Authenticated users can read laundry rooms" ON laundry_rooms;
DROP POLICY IF EXISTS "Authenticated users can read laundry slots" ON laundry_slots;
DROP POLICY IF EXISTS "Tenant can read own bookings" ON laundry_bookings;
DROP POLICY IF EXISTS "Tenant can insert own bookings" ON laundry_bookings;
DROP POLICY IF EXISTS "Tenant can update own bookings" ON laundry_bookings;
...
CREATE UNIQUE INDEX IF NOT EXISTS idx_laundry_bookings_one_active_per_slot
  ON laundry_bookings(laundry_slot_id)
  WHERE status = 'active';
