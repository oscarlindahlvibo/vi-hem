...
CREATE POLICY "Users can read permitted laundry rooms"
  ON laundry_rooms FOR SELECT
  TO authenticated
  USING (
    get_my_role() = ANY (ARRAY['staff', 'admin', 'superadmin'])
    OR property_id IN (
...
CREATE POLICY "Users can read permitted laundry slots"
  ...
  USING (
    laundry_room_id IN (
      ...
      WHERE get_my_role() = ANY (ARRAY['staff', 'admin', 'superadmin'])
...
CREATE POLICY "Users can read permitted laundry bookings"
...
CREATE POLICY "Users can insert permitted laundry bookings"
...
CREATE POLICY "Users can update permitted laundry bookings"
