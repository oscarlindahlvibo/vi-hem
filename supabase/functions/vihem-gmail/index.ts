...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (action === "status") return out({ ok: true, configured: Boolean(credentials()), scope: SCOPE, accounts });
    ...
    if (action === "test") {
      ...
      try { await gmail(account.email, "/profile"); await updateTest(db, account.id, "ok", ""); await audit(db, profile, "connection_tested", account.id, "ok", ""); return out({ ok: true, code: "OK" }); }
    ...
    if (action === "search") {
      ...
      const results = (await Promise.all(selected.map(async account => { try { return { account, results: await searchMailbox(account, query, mode), error: null }; } catch (e) { const code = googleCode(e); return { account, results: [], error: { code, message: friendly(code) } }; } }))).map(x => ({ account: safeAccount(x.account), results: x.results, error: x.error }));
    ...
    if (!account) return out({ error: "Mailboxen hittades inte.", code: "ACCOUNT_NOT_FOUND" }, 404);
    if (action === "message") { const message = await gmail(account.email, `/messages/${encodeURIComponent(String(body.message_id || ""))}?format=full`); await audit(db, profile, "message_read", account.id, "ok", {}); return out({ ok: true, message: normalizeMessage(message) }); }
    if (action === "attachment") { const attachment = await getAttachment(account.email, String(body.message_id || ""), String(body.attachment_id || "")); await audit(db, profile, "attachment_downloaded", account.id, "ok", {}); return out({ ok: true, filename: String(body.filename || "bilaga"), mime_type: String(body.mime_type || "application/octet-stream"), data_base64: attachment }); }
...
function credentials() { const raw = Deno.env.get("GOOGLE_WORKSPACE_SERVICE_ACCOUNT_JSON") || Deno.env.get("GOOGLE_WORKSPACE_SERVICE_ACCOUNT_JSON_BASE64"); if (!raw) return null; try { return JSON.parse(raw.startsWith("{") ? raw : atob(raw)); } catch { return null; } }
async function token(subject: string) { const c = credentials(); if (!c?.client_email || !c?.private_key) throw new Error("GOOGLE_CREDENTIALS_MISSING"); const now = Math.floor(Date.now() / 1000); const enc = (v: unknown) => b64url(new TextEncoder().encode(JSON.stringify(v))); const unsigned = `${enc({ alg: "RS256", typ: "JWT" })}.${enc({ iss: c.client_email, scope: SCOPE, aud: "https://oauth2.googleapis.com/token", sub: subject, iat: now, exp: now + 3600 })}`; const key = await crypto.subtle.importKey("pkcs8", pem(c.private_key), { name: "RSASSA-PKCS1-v1_5", hash: "SHA-256" }, false, ["sign"]); const sig = await crypto.subtle.sign("RSASSA-PKCS1-v1_5", key, new TextEncoder().encode(unsigned)); const response = await fetch("https://oauth2.googleapis.com/token", { method: "POST", headers: { "content-type": "application/x-www-form-urlencoded" }, body: `grant_type=urn%3Aietf%3Aparams%3Aoauth%3Agrant-type%3Ajwt-bearer&assertion=${unsigned}.${b64url(new Uint8Array(sig))}` }); if (!response.ok) throw new Error("GOOGLE_TOKEN_FAILED"); return (await response.json()).access_token as string; }
async function gmail(user: string, path: string) { const response = await fetch(`https://gmail.googleapis.com/gmail/v1/users/${encodeURIComponent(user)}${path}`, { headers: { Authorization: `Bearer ${await token(user)}` } }); if (!response.ok) { const e = await response.text(); throw new Error(`${response.status}:${e.slice(0, 200)}`); } return response.json(); }
async function getAttachment(user: string, message: string, attachment: string) { const data = await gmail(user, `/messages/${encodeURIComponent(message)}/attachments/${encodeURIComponent(attachment)}`); return data.data || ""; }
async function searchMailbox(account: MailAccount, query: string, mode: string) { const q = mode === "invoice" ? `${query} (invoice OR faktura OR kvitto OR receipt)`.trim() : query; const list = await gmail(account.email, `/messages?q=${encodeURIComponent(q)}&maxResults=20&includeSpamTrash=false`); const messages = await Promise.all((list.messages || []).slice(0, 20).map((m: { id: string }) => gmail(account.email, `/messages/${m.id}?format=metadata&metadataHeaders=Subject&metadataHeaders=From&metadataHeaders=To&metadataHeaders=Date&metadataHeaders=Cc`))); return messages.map((m: any) => { const headers = Object.fromEntries((m.payload?.headers || []).map((h: any) => [h.name.toLowerCase(), h.value])); const text = `${headers.subject || ""} ${headers.from || ""} ${m.snippet || ""}`; const score = scoreText(text, query, mode); return { id: m.id, thread_id: m.threadId, subject: headers.subject || "(utan ämne)", from: headers.from || "", to: headers.to || "", date: headers.date || "", snippet: m.snippet || "", score, has_attachments: Boolean(m.sizeEstimate && m.sizeEstimate > 0) }; }).sort((a: any, b: any) => b.score - a.score); }
