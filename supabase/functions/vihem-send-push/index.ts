...
Deno.serve(async (request) => {
  ...
  // Matches vihem-accounted-healthcheck's pattern: the secret lives in
  // vihem_system_settings (generated by the dispatch migration), not an
  // env var -- there's no fixed value to configure at deploy time since
  // the migration generates it fresh in the database.
  const providedSecret = request.headers.get("X-VIHEM-Push-Secret") || "";
  const { data: settingsRow } = await supabase
    .from("vihem_system_settings")
    .select("value")
    .eq("key", "push_notification_dispatch")
    .maybeSingle();
  const expectedSecret = settingsRow?.value && typeof settingsRow.value === "object"
    ? String((settingsRow.value as { secret?: string }).secret || "")
    : "";
