...
const PRODUCTION_SIGN_URL = "https://api.banksignering.se/api/sign";
const PRODUCTION_COLLECT_URL = "https://api.banksignering.se/api/collectstatus";
...
async function startProvider(s: any, visible: string, nonVisible: string, req: Request) { const credentials = await credentialsFor(s); const endUserIp = (req.headers.get("x-forwarded-for") || "0.0.0.0").split(",")[0].trim(); console.log("vihem-bankid startProvider debug", { endUserIp, xff: req.headers.get("x-forwarded-for"), xRealIp: req.headers.get("x-real-ip"), cfConnectingIp: req.headers.get("cf-connecting-ip"), environment: s?.environment }); const response = await fetch(signUrl(s), { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ ...credentials, endUserIp, userVisibleData: visible, userNonVisibleData: nonVisible, getQr: true }) }); const data = await response.json().catch(() => ({})); if (!response.ok) throw new Error(data?.message || data?.Message || `BankSignering start misslyckades (${response.status}).`); const r = unwrapProviderResponse(data); const orderRef = pickString(r, ["OrderRef", "orderRef", "order_ref"]); const autoStartToken = pickString(r, ["AutoStartToken", "autoStartToken", "auto_start_token", "autostarttoken"]); const qrImage = pickString(r, ["QrImage", "qrImage", "qr_image"]) || null; if (!orderRef) throw new Error("BankSignering returnerade inget OrderRef."); return { orderRef, autoStartToken, autoStartUrl: bankIdAutoStartUrl(autoStartToken), qrImage }; }
async function collectProvider(s: any, orderRef: string) { const credentials = await credentialsFor(s); const response = await fetch(collectUrl(s), { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify({ ...credentials, orderRef }) }); const data = await response.json().catch(() => ({})); if (!response.ok) throw new Error(data?.message || data?.Message || `BankSignering collect misslyckades (${response.status}).`); const r = unwrapProviderResponse(data); const status = String(pickValue(r, ["Status", "status"]) || "pending").toLowerCase(); const completion = pickValue(r, ["CompletionData", "completionData", "completion_data"]); return { orderRef, status: status === "complete" ? "complete" : status === "failed" ? "failed" : "pending", hintCode: pickValue(r, ["HintCode", "hintCode", "hint_code"]), completionData: completion ? { user: { personalNumber: completion.User?.PersonalNumber || completion.user?.personalNumber || "", name: completion.User?.Name || completion.user?.name || "", givenName: completion.User?.GivenName || completion.user?.givenName || "", surname: completion.User?.Surname || completion.user?.surname || "" }, signature: completion.Signature || completion.signature || "", ocspResponse: completion.OcspResponse || completion.ocspResponse || "" } : undefined }; }
...
function signUrl(s: any) { const raw = s.environment === "test" ? (Deno.env.get("BANKSIGN_TEST_SIGN_URL") || TEST_SIGN_URL) : (Deno.env.get("BANKSIGN_PRODUCTION_SIGN_URL") || PRODUCTION_SIGN_URL); return providerEndpoint(raw, "sign"); }
function collectUrl(s: any) { const raw = s.environment === "test" ? (Deno.env.get("BANKSIGN_TEST_COLLECT_URL") || TEST_COLLECT_URL) : (Deno.env.get("BANKSIGN_PRODUCTION_COLLECT_URL") || PRODUCTION_COLLECT_URL); return providerEndpoint(raw, "collectstatus"); }
function providerEndpoint(raw: string, action: "sign" | "collectstatus") { const trimmed = String(raw || "").trim().replace(/\/+$/, ""); if (!trimmed) throw new Error("BankSignering URL saknas."); if (/\/api\/(sign|collectstatus)$/i.test(trimmed)) return trimmed.replace(/\/api\/(sign|collectstatus)$/i, `/api/${action}`); if (/\/api$/i.test(trimmed)) return `${trimmed}/${action}`; return `${trimmed}/api/${action}`; }
function unwrapProviderResponse(data: any) { return data?.apiCallResponse?.Response || data?.apiCallResponse?.response || data?.ApiCallResponse?.Response || data?.ApiCallResponse?.response || data?.Response || data?.response || data; }
function pickValue(source: any, keys: string[]) { for (const key of keys) { const value = source?.[key]; if (value !== undefined && value !== null) return value; } return undefined; }
function pickString(source: any, keys: string[]) { const value = pickValue(source, keys); return value === undefined || value === null ? "" : String(value); }
function bankIdAutoStartUrl(token: string) { return token ? `https://app.bankid.com/?autostarttoken=${encodeURIComponent(token)}` : ""; }
