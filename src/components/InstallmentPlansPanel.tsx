...
const emptyForm: PlanForm = {
  companyId: '', customerId: '', firstDueDate: today, installmentCount: '3', intervalMonths: '1', dayOfMonth: String(new Date().getDate()), emailLeadDays: '30',
  terms: '', notes: '',
};
...
function emailStatusLabel(status: InstallmentSchedule['email_status']) {
  return ({ pending: 'Schemalat', queued: 'I kö', sent: 'Skickat', failed: 'Misslyckat', skipped: 'Avaktiverat' } as Record<string, string>)[status] ?? status;
}
...
export function InstallmentPlansPanel({ organisationId, companies, customers, invoices, userId }: Props) {
  ...
  const saveCustomer = async () => {
    ...
    const payload = {
      ...
      organisation_number: customerDraft.customerType === 'private' ? '' : customerDraft.organisationNumber.trim(),
      personal_number: customerDraft.customerType === 'private' ? customerDraft.personalNumber.trim() : '',
  ...
  const createPlan = async () => {
    ...
    const { data: plan, error: planError } = await supabase.from('vihem_installment_plans').insert({
      ...
      installment_count: count, first_due_date: form.firstDueDate, interval_months: Math.max(1, Number(form.intervalMonths) || 1), email_lead_days: Math.max(0, Number(form.emailLeadDays) || 0),
  ...
  return <div className="installment-plans-panel installment-plans-layout gap-5">
    <div className="space-y-4">
      ...
      {showCreate && <Card className="overflow-hidden border-blue-100 p-0 shadow-md"><div className="bg-gradient-to-r from-blue-700 to-indigo-600 px-5 py-5 text-white"><div className="flex items-center gap-3"><div className="rounded-xl bg-white/15 p-2"><ReceiptText className="h-5 w-5" /></div><div><h3 className="text-lg font-bold">Ny avbetalningsplan</h3><p className="mt-0.5 text-sm text-blue-100">Samla flera underlag i en tydlig plan.</p></div></div></div><div className="space-y-6 p-5">
        ...
        <section><p className="mb-3 text-xs font-bold uppercase tracking-wider text-blue-700">4. Betalningsupplägg</p><div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-5"><Input label="Första förfallodag" type="date" value={form.firstDueDate} onChange={event => updateForm('firstDueDate', event.target.value)} /><Input label="Antal delar" type="number" min="1" value={form.installmentCount} onChange={event => updateForm('installmentCount', event.target.value)} /><Input label="Intervall (mån)" type="number" min="1" value={form.intervalMonths} onChange={event => updateForm('intervalMonths', event.target.value)} /><Input label="Dag i månaden" type="number" min="1" max="31" value={form.dayOfMonth} onChange={event => updateForm('dayOfMonth', event.target.value)} /><Input label="E-post dagar före" type="number" min="0" value={form.emailLeadDays} onChange={event => updateForm('emailLeadDays', event.target.value)} /></div><p className="mt-2 text-xs text-slate-500">Standard är 30 dagar före varje förfallodag. Sätt 0 för utskick på förfallodagen.</p></section>
