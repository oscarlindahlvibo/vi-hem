...
import {
  ...
  Truck, Package, FileSpreadsheet, Mail, FileSignature, Sparkles,
} from 'lucide-react';
...
export function Layout({ children, currentPage, onNavigate, notificationCount = 0, chatNotificationCount = 0, enabledModules = {} }: LayoutProps) {
  ...
  const navGroups: NavGroup[] = [
    ...
    { label: 'Fastigheter & boende', icon: <Building2 className="w-5 h-5" />, items: [
      ...
      { label: 'Avtal V2 (beta)', icon: <FileSignature className="w-5 h-5" />, page: 'agreements-v2', roles: ['staff', 'admin'] },
      { label: 'Avtalsskapare V3 (test)', icon: <Sparkles className="w-5 h-5" />, page: 'agreements-v3', roles: ['staff', 'admin'] },
