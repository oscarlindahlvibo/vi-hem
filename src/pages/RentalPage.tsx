...
type Booking = { id: string; public_reference: string; product_id: string | null; status: string; payment_status?: string; start_at: string; end_at: string; total: number; customer_id: string | null; customer?: RentalCustomer | null; customer_notes?: string; internal_notes?: string };
...
const emptyBooking = { product_id: '', start_at: '', end_at: '', quantity: '1', first_name: '', last_name: '', company_name: '', email: '', phone: '', customer_notes: '', internal_notes: '', status: 'confirmed', payment_status: 'unpaid' };
type BookingForm = typeof emptyBooking;
...
function startOfCalendarWeek(value: Date) { const result = new Date(value); result.setHours(0, 0, 0, 0); const day = result.getDay() || 7; result.setDate(result.getDate() - day + 1); return result; }

function RentalCalendar({ bookings, blocks, products, onOpenBooking }: { bookings: Booking[]; blocks: Block[]; products: Product[]; onOpenBooking: (booking: Booking) => void }) {
  const [anchor, setAnchor] = useState(() => startOfCalendarWeek(new Date()));
  const days = Array.from({ length: 14 }, (_, index) => { const day = new Date(anchor); day.setDate(anchor.getDate() + index); return day; });
  const productIds = [...new Set([...bookings.map(booking => booking.product_id), ...blocks.map(block => block.product_id)].filter(Boolean))] as string[];
  const productRows = productIds.map(id => ({ id, name: products.find(product => product.id === id)?.name || 'Produkt' }));
  const position = (value: string) => Math.max(0, Math.min(13, Math.floor((new Date(value).getTime() - days[0].getTime()) / 86400000)));
  const span = (start: string, end: string) => Math.max(1, Math.min(14 - position(start), Math.ceil((new Date(end).getTime() - Math.max(new Date(start).getTime(), days[0].getTime())) / 86400000)));
  return <Card className="overflow-hidden"><div className="flex flex-wrap items-center justify-between gap-3 border-b border-slate-200 p-4"><div><h2 className="font-semibold">Uthyrningskalender</h2><p className="text-sm text-slate-500">Bokningar och interna spärrar per produkt. Klicka på en bokning för detaljer.</p></div><div className="flex gap-2"><Button size="sm" variant="secondary" onClick={() => { const next = new Date(anchor); next.setDate(anchor.getDate() - 14); setAnchor(next); }}>← Föregående</Button><Button size="sm" variant="secondary" onClick={() => setAnchor(startOfCalendarWeek(new Date()))}>Idag</Button><Button size="sm" variant="secondary" onClick={() => { const next = new Date(anchor); next.setDate(anchor.getDate() + 14); setAnchor(next); }}>Nästa →</Button></div></div><div className="overflow-x-auto"><div className="min-w-[1120px]"><div className="grid border-b border-slate-200 bg-slate-50" style={{ gridTemplateColumns: '220px repeat(14, minmax(64px, 1fr))' }}><div className="p-3 text-xs font-semibold uppercase tracking-wide text-slate-500">Produkt</div>{days.map(day => <div key={dateKey(day)} className={`border-l border-slate-200 p-2 text-center text-xs ${dateKey(day) === dateKey(new Date()) ? 'bg-blue-50 text-blue-700' : 'text-slate-500'}`}><div>{day.toLocaleDateString('sv-SE', { weekday: 'short' })}</div><strong>{day.getDate()} {day.toLocaleDateString('sv-SE', { month: 'short' })}</strong></div>)}</div>{productRows.length === 0 ? <div className="p-10 text-center text-sm text-slate-500">Inga bokningar eller spärrar att visa.</div> : productRows.map(row => { const rowBookings = bookings.filter(booking => booking.product_id === row.id); const rowBlocks = blocks.filter(block => block.product_id === row.id); return <div key={row.id} className="grid min-h-[92px] border-b border-slate-200" style={{ gridTemplateColumns: '220px repeat(14, minmax(64px, 1fr))' }}><div className="border-r border-slate-200 p-3"><p className="font-semibold text-slate-800">{row.name}</p><p className="text-xs text-slate-500">{rowBookings.length} bokning{rowBookings.length === 1 ? '' : 'ar'}</p></div><div className="relative col-span-14 grid grid-cols-14 bg-white" style={{ gridColumn: '2 / span 14' }}>{days.map(day => <div key={dateKey(day)} className="border-l border-slate-100" />)}{rowBookings.map(booking => <button key={booking.id} className="absolute z-10 mx-1 mt-3 min-h-9 overflow-hidden rounded-lg bg-blue-600 px-2 py-1 text-left text-xs font-semibold text-white shadow-sm hover:bg-blue-700" style={{ left: `${(position(booking.start_at) / 14) * 100}%`, width: `${(span(booking.start_at, booking.end_at) / 14) * 100}%` }} onClick={() => onOpenBooking(booking)}><span className="block truncate">{booking.public_reference}</span><span className="block truncate text-[10px] font-normal">{dateTime(booking.start_at)} – {dateTime(booking.end_at)}</span></button>)}{rowBlocks.map(block => <div key={block.id} className="absolute bottom-2 z-10 mx-1 h-7 overflow-hidden rounded bg-amber-100 px-2 py-1 text-[11px] font-semibold text-amber-800" style={{ left: `${(position(block.start_at) / 14) * 100}%`, width: `${(span(block.start_at, block.end_at) / 14) * 100}%` }}>{block.reason || 'Spärr'}</div>)}</div></div>; })}</div></div></Card>;
}

function BookingEditorModal({ open, value, onChange, onClose, onSave, saving }: { open: boolean; value: BookingForm; onChange: (value: BookingForm) => void; onClose: () => void; onSave: () => void; saving: boolean }) {
  return <Modal open={open} onClose={onClose} title="Redigera bokning" size="lg"><div className="space-y-4"><div className="grid gap-4 sm:grid-cols-2"><Input label="Start" type="datetime-local" value={value.start_at} onChange={event => onChange({ ...value, start_at: event.target.value })} /><Input label="Slut" type="datetime-local" value={value.end_at} onChange={event => onChange({ ...value, end_at: event.target.value })} /><Select label="Bokningsstatus" value={value.status} onChange={event => onChange({ ...value, status: event.target.value })} options={[{ value: 'pending', label: 'Väntar' }, { value: 'confirmed', label: 'Bekräftad' }, { value: 'active', label: 'Pågår' }, { value: 'completed', label: 'Avslutad' }, { value: 'cancelled', label: 'Avbokad' }]} /><Select label="Betalstatus" value={value.payment_status} onChange={event => onChange({ ...value, payment_status: event.target.value })} options={[{ value: 'unpaid', label: 'Obetald' }, { value: 'pending', label: 'Väntar betalning' }, { value: 'paid', label: 'Betald' }, { value: 'partially_refunded', label: 'Delvis återbetalad' }, { value: 'refunded', label: 'Återbetalad' }]} /></div><div className="border-t border-slate-200 pt-4"><h3 className="font-semibold">Kunduppgifter</h3><div className="mt-3 grid gap-4 sm:grid-cols-2"><Input label="Förnamn" value={value.first_name} onChange={event => onChange({ ...value, first_name: event.target.value })} /><Input label="Efternamn" value={value.last_name} onChange={event => onChange({ ...value, last_name: event.target.value })} /><Input label="Företag" value={value.company_name} onChange={event => onChange({ ...value, company_name: event.target.value })} /><Input label="E-post" type="email" value={value.email} onChange={event => onChange({ ...value, email: event.target.value })} /><Input label="Telefon" value={value.phone} onChange={event => onChange({ ...value, phone: event.target.value })} /></div></div><Textarea label="Kundanteckning" rows={3} value={value.customer_notes} onChange={event => onChange({ ...value, customer_notes: event.target.value })} /><Textarea label="Intern anteckning" rows={3} value={value.internal_notes} onChange={event => onChange({ ...value, internal_notes: event.target.value })} /><div className="flex justify-end gap-2"><Button variant="secondary" onClick={onClose}>Avbryt</Button><Button onClick={onSave} loading={saving}>Spara ändringar</Button></div></div></Modal>;
}
...
export function RentalPage({ onNavigate: _onNavigate }: { onNavigate: (page: string) => void }) {
  ...
  const [provisioningCustomerId, setProvisioningCustomerId] = useState<string | null>(null);
  ...
  async function fetchData() {
    ...
    const [productResult, assetResult, priceResult, bookingResult, blockResult, customerResult, settingsResult] = await Promise.all([
      ...
      supabase.from('vihem_rental_bookings').select('id,public_reference,status,payment_status,start_at,end_at,total,customer_id,customer_notes,internal_notes,customer:vihem_rental_customers(id,first_name,last_name,company_name,email,phone,city,created_at),vihem_rental_booking_items(product_id,quantity)').eq('organisation_id', organisationId).order('start_at'),
    ...
    setBookings(((bookingResult.data || []) as any[]).map(row => ({ ...row, product_id: row.vihem_rental_booking_items?.[0]?.product_id || null, customer: Array.isArray(row.customer) ? row.customer[0] : row.customer })));
  ...
  function openNewBooking(productId = products[0]?.id || '') { setEditingBookingId(null); setBookingForm({ ...emptyBooking, product_id: productId }); setBookingModal(true); }
  function openEditBooking(booking: Booking) { const customer = booking.customer; setEditingBookingId(booking.id); setBookingForm({ ...emptyBooking, product_id: booking.product_id || '', start_at: booking.start_at.slice(0, 16), end_at: booking.end_at.slice(0, 16), first_name: customer?.first_name || '', last_name: customer?.last_name || '', company_name: customer?.company_name || '', email: customer?.email || '', phone: customer?.phone || '', customer_notes: booking.customer_notes || '', internal_notes: booking.internal_notes || '', status: booking.status, payment_status: booking.payment_status || 'unpaid' }); setBookingDetailModal(true); }
  ...
  async function saveBooking() {
    ...
    setSaving(true); setError('');
    if (editingBookingId) {
      const result = await supabase.from('vihem_rental_bookings').update({ start_at: new Date(bookingForm.start_at).toISOString(), end_at: new Date(bookingForm.end_at).toISOString(), status: bookingForm.status, payment_status: bookingForm.payment_status, customer_notes: bookingForm.customer_notes, internal_notes: bookingForm.internal_notes, updated_at: new Date().toISOString() }).eq('id', editingBookingId).select('id').single();
      if (result.error) { setSaving(false); setError(result.error.message); return; }
      const current = bookings.find(booking => booking.id === editingBookingId);
      if (current?.customer_id) await supabase.from('vihem_rental_customers').update({ first_name: bookingForm.first_name, last_name: bookingForm.last_name, company_name: bookingForm.company_name, email: bookingForm.email, phone: bookingForm.phone, updated_at: new Date().toISOString() }).eq('id', current.customer_id);
      setSaving(false); setBookingModal(false); setEditingBookingId(null); setBookingForm(emptyBooking); await fetchData(); return;
    }
    ...
    setSaving(false);
    if (result.error) { setError(result.error.message); return; }
    setBookingModal(false); setBookingForm(emptyBooking); setEditingBookingId(null); await fetchData();
  }
  ...
  async function provisionCustomerPortal(customer: RentalCustomer) {
    if (!organisationId || !customer.email) { setError('Kunden måste ha en e-postadress.'); return; }
    setProvisioningCustomerId(customer.id); setError('');
    const { error: portalError } = await supabase.functions.invoke('vihem-rental-customer-portal', { body: { action: 'provision', organisation_id: organisationId, customer_id: customer.id } });
    setProvisioningCustomerId(null);
    setError(portalError ? portalError.message : `Kundinloggning skickad till ${customer.email}.`);
  }
  ...
  return (
    <div className="space-y-5">
      ...
      {tab === 'calendar' && <RentalCalendar bookings={activeBookings} blocks={blocks} products={products} onOpenBooking={openEditBooking} />}
      {tab === 'bookings' && <Card className="p-4"><div className="flex items-center justify-between gap-3"><div><h2 className="font-semibold">Öppna bokning</h2><p className="text-sm text-slate-500">Välj en bokning för att se kunduppgifter och ändra detaljer.</p></div><Select label="Bokning" value="" onChange={event => { const booking = bookings.find(item => item.id === event.target.value); if (booking) openEditBooking(booking); }} options={[{ value: '', label: 'Välj bokning…' }, ...bookings.map(booking => ({ value: booking.id, label: `${booking.public_reference} · ${productById.get(booking.product_id || '')?.name || 'Produkt'}` }))]} /></div></Card>}
      ...
      {tab === 'customers' && <Card className="overflow-hidden"><div className="flex items-center justify-between gap-3 border-b border-slate-200 p-4"><div><h2 className="font-semibold">Uthyrningskunder</h2><p className="text-sm text-slate-500">Kunduppgifter återanvänds vid interna och publika bokningar. En publik bokning skapar portalinbjudan automatiskt.</p></div><Button onClick={() => setCustomerModal(true)}><Plus className="h-4 w-4" /> Ny kund</Button></div>{customers.length === 0 ? <EmptyState title="Inga kunder ännu" /> : <div className="divide-y divide-slate-100">{customers.map(customer => <div key={customer.id} className="flex flex-col gap-3 p-4 md:flex-row md:items-center md:justify-between"><div><p className="font-semibold">{[customer.first_name, customer.last_name].filter(Boolean).join(' ') || customer.company_name || 'Namnlös kund'}</p><p className="text-sm text-slate-500">{customer.company_name && customer.company_name !== `${customer.first_name} ${customer.last_name}` ? `${customer.company_name} · ` : ''}{customer.email || customer.phone || 'Ingen kontaktuppgift'}</p></div><div className="flex items-center gap-3"><span className="text-sm text-slate-500">{customer.city || 'Ingen ort'}</span><Button size="sm" variant="secondary" onClick={() => void provisionCustomerPortal(customer)} loading={provisioningCustomerId === customer.id} disabled={!customer.email}>Skicka kundinloggning</Button></div></div>)}</div>}</Card>}
      ...
      <BookingEditorModal open={bookingDetailModal} value={bookingForm} onChange={setBookingForm} onClose={() => { setBookingDetailModal(false); setEditingBookingId(null); }} onSave={saveBooking} saving={saving} />
