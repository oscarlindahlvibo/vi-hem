...
const columns: Record<ImportKind, string[]> = {
  ...
  apartments: ['property_name', 'property_address', 'apartment_number', 'size', 'rooms', 'rent', 'floor', 'storage', 'parking', 'balcony', 'balcony_size', 'status', 'storage_id', 'parking_spot_id', 'cellar_id', 'mailbox_id', 'lock_cylinder_id', 'door_code', 'entry_code_updated_at', 'key_ids', 'network_outlet_ids', 'electricity_fuse_box', 'electricity_meter_id', 'water_meter_id', 'heat_meter_id', 'ventilation_unit_id', 'last_renovation_year', 'technical_notes', 'company_name', 'company_org_number'],
...
const templateRows: Record<ImportKind, ImportRow[]> = {
  ...
  apartments: [{ property_name: 'Ekängsvägen 1', property_address: 'Ekängsvägen 1', apartment_number: '1001', size: '72', rooms: '3', rent: '8500', floor: '1', storage: 'Förråd 1001', parking: 'P-12', balcony: 'true', balcony_size: '8', status: 'vacant', storage_id: '', parking_spot_id: 'P-12', cellar_id: '', mailbox_id: '', lock_cylinder_id: '', door_code: '', entry_code_updated_at: '', key_ids: '[]', network_outlet_ids: '[]', electricity_fuse_box: '', electricity_meter_id: '', water_meter_id: '', heat_meter_id: '', ventilation_unit_id: '', last_renovation_year: '2020', technical_notes: '', company_name: '', company_org_number: '' }],
  ...
  async function importRows() {
    ...
    try {
      if (kind === 'properties') {
        ...
        const [properties, apartments, companies] = await Promise.all([supabase.from('vihem_properties').select('id,name,address').eq('organisation_id', user.organisation_id), supabase.from('vihem_apartments').select('id,property_id,apartment_number').eq('organisation_id', user.organisation_id), supabase.from('vihem_companies').select('id,name,legal_name,organisation_number').eq('organisation_id', user.organisation_id)]);
        for (const [index, row] of rows.entries()) {
          ...
          const companyName = clean(row.company_name); const companyNumber = clean(row.company_org_number);
          const company = companyName || companyNumber ? (companies.data || []).find(item => (companyNumber && item.organisation_number === companyNumber) || (companyName && [item.name, item.legal_name].some(value => value.toLowerCase() === companyName.toLowerCase()))) : null;
          if ((companyName || companyNumber) && !company) { output.push({ row: index + 2, status: 'error', message: `Hittar inte bolaget "${companyName || companyNumber}".` }); continue; }
