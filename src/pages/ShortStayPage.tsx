...
export function ShortStayPage({ onNavigate }: ShortStayPageProps) {
  ...
  const stats = useMemo(() => {
    ...
    // A still-dirty/in_progress item stays here no matter how old -- it
    // needs to stay swipeable until someone actually clears it, same as
    // the TV screen's own "Ej städade" panel (ScreenDisplayPage.tsx) shows
    // it forever until resolved. Only an already-clean item is capped to
    // "today", as a same-day confirmation rather than a growing history.
