...
export function AdminPropertiesPage({ onNavigate: _onNavigate }: AdminPropertiesPageProps) {
  ...
  const [orgLimits, setOrgLimits] = useState<{ max_apartments: number } | null>(null);
                ...
                return (
                  <Card key={property.id} className="cursor-pointer hover:shadow-md transition-shadow">
                    <div className="flex items-start justify-between mb-4">
                      ...
                      <button onClick={(e) => { e.stopPropagation(); openEditPropertyModal(property); }} className="p-2 hover:bg-slate-100 rounded-lg">
                        <Edit2 className="w-4 h-4 text-slate-600" />
                      </button>
                    </div>
                    {property.description && <p className="text-sm text-slate-600 mb-4">{property.description}</p>}
                    <div className="flex items-center gap-6 py-3 border-t border-b border-slate-100 text-sm">
