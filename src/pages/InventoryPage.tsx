...
type Item = { id: string; article_number: string; name: string; category: string; supplier: string; barcode: string; qr_identifier: string; unit: string; purchase_price: number; minimum_stock: number; target_stock: number; reorder_quantity: number; notes: string; active: boolean };
...
type Form = { article_number: string; name: string; category: string; supplier: string; barcode: string; unit: string; purchase_price: string; minimum_stock: string; target_stock: string; reorder_quantity: string; notes: string };

const emptyForm: Form = { article_number: '', name: '', category: '', supplier: '', barcode: '', unit: 'st', purchase_price: '0', minimum_stock: '0', target_stock: '0', reorder_quantity: '0', notes: '' };
...
export function InventoryPage({ onNavigate }: { onNavigate: (page: string) => void }) {
  ...
  const [tab, setTab] = useState<'overview' | 'items' | 'locations' | 'transactions'>('overview');
  ...
  async function load() {
    ...
    const [itemsRes, locationsRes, balancesRes, txRes, projectsRes, workOrdersRes] = await Promise.all([
    ...
    setItems((itemsRes.data || []) as Item[]); setLocations((locationsRes.data || []) as Location[]); setBalances((balancesRes.data || []) as Balance[]); setTransactions((txRes.data || []) as Tx[]);
  ...
  function openMovement(itemId = '') { setMovement({ item_id: itemId || items[0]?.id || '', quantity: '1', type: 'stock_out', source: balances.find(row => row.item_id === (itemId || items[0]?.id))?.location_id || locations[0]?.id || '', destination: locations[0]?.id || '', project: '', workOrder: '', notes: '' }); setError(''); setMessage(''); setMovementModal(true); }
  async function saveItem() {
    if (!user?.organisation_id || !isAdmin || !form.name.trim()) { setError('Ange ett artikelnamn.'); return; }
    const payload = { organisation_id: user.organisation_id, article_number: form.article_number.trim(), name: form.name.trim(), category: form.category.trim(), supplier: form.supplier.trim(), barcode: form.barcode.trim(), qr_identifier: form.barcode.trim() || crypto.randomUUID(), unit: form.unit.trim() || 'st', purchase_price: Number(form.purchase_price) || 0, minimum_stock: Number(form.minimum_stock) || 0, target_stock: Number(form.target_stock) || 0, reorder_quantity: Number(form.reorder_quantity) || 0, notes: form.notes.trim(), created_by: user.id };
  ...
  async function applyMovement() {
    ...
    if (movementError) { setError(movementError.message); return; } setMovementModal(false); setMessage('Lagerhändelsen sparades.'); await load();
  }
  ...
  return <div className="mx-auto max-w-7xl px-4 py-6 sm:px-6 lg:px-8">
    ...
    <div className="mb-5 flex flex-wrap gap-2">{[['overview','Översikt'],['items','Artiklar'],['locations','Lagerplatser'],['transactions','Historik']].map(([value,label]) => <Button key={value} variant={tab === value ? 'primary' : 'secondary'} size="sm" onClick={() => setTab(value as typeof tab)}>{label}</Button>)}</div>
