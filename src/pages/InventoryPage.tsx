...
export function InventoryPage({ onNavigate }: { onNavigate: (page: string) => void }) {
  ...
  const [itemLookupMode, setItemLookupMode] = useState<'url' | 'text'>('url');
  const [itemLookupUrl, setItemLookupUrl] = useState('');
  const [itemLookupText, setItemLookupText] = useState('');
  const [itemLookupLoading, setItemLookupLoading] = useState(false);
  const [itemLookupError, setItemLookupError] = useState('');
  const [itemLookupNote, setItemLookupNote] = useState('');
  ...
  function resetItemLookup() { setItemLookupMode('url'); setItemLookupUrl(''); setItemLookupText(''); setItemLookupError(''); setItemLookupNote(''); }
  async function lookupItem() {
    const body = itemLookupMode === 'url' ? { url: itemLookupUrl.trim() } : { text: itemLookupText.trim() };
    if (itemLookupMode === 'url' && !itemLookupUrl.trim()) { setItemLookupError('Klistra in en länk först.'); return; }
    if (itemLookupMode === 'text' && !itemLookupText.trim()) { setItemLookupError('Klistra in text först.'); return; }
    setItemLookupLoading(true); setItemLookupError(''); setItemLookupNote('');
    try {
      const { data, error: err } = await supabase.functions.invoke('vihem-inventory-lookup-item', { body });
      if (err) {
      ...
      setForm(current => ({
        ...current,
        name: str(found.name) || current.name,
        article_number: str(found.article_number) || current.article_number,
        category: str(found.category) || current.category,
        supplier: str(found.supplier) || current.supplier,
        barcode: str(found.barcode) || current.barcode,
        unit: str(found.unit) || current.unit,
        purchase_price: num(found.purchase_price) || current.purchase_price,
        notes: current.notes || str(found.notes),
        image_url: str(data?.image_url) || current.image_url,
      }));
      setItemLookupNote(str(data?.image_url) ? 'Fälten och en bild är ifyllda -- kontrollera dem innan du sparar.' : 'Fälten är ifyllda -- kontrollera dem innan du sparar.');
    } catch (err) {
      setItemLookupError(describeError(err));
    } finally {
  ...
  return <div className="mx-auto max-w-7xl px-4 py-6 sm:px-6 lg:px-8">
    ...
    <Modal open={itemModal} onClose={() => { setItemModal(false); setEditingItem(null); }} title={editingItem ? "Redigera lagerartikel" : "Ny lagerartikel"} size="lg">
